...
- **Hub MCP backend deploys avoid market hours (preferred).** Changes to `backend/hub_mcp/` (or anything triggering a Railway redeploy of the FastMCP service) are PREFERABLY not pushed during 09:30–16:00 ET, Mon–Fri, unless fixing an active production issue. OAuth state persistence shipped 2026-05-27 (Brief D rev2, `3d6dd94`), so a redeploy no longer forces a manual Claude.ai reconnect — which is why this downgraded from "should not" to "preferred." The residual reason to still avoid market hours: each redeploy drops the hub for ~60–170s while FastMCP restarts, which can interrupt an active Olympus committee pass mid-session. (This is a separate, narrower consideration than the general RTH blackout below — not affected by that ruling.)
- **No general 07:30–14:00 MT push/deploy blackout — RETIRED, ruled by Nick 2026-07-16.** An informal "no push to main during this window" convention operated for about a week (never written here — tracked only in session memory/handoff docs) after a 2026-07-14 incident where a blackout-window deploy's ~60-170s outage window coincided with 3 inbound TradingView webhook POSTs; since TradingView doesn't retry, those signals were lost rather than delayed. That incident and a follow-up 2026-07-16 blackout-window push (with a same-day DB sweep finding no comparable signal loss) led to explicit rulings both times ("run what you need to" 7/14; "Push now. Blackouts don't matter" 7/16, general not scoped to one push). Given three options tonight (retire / keep as informal-only / replace with a narrower batch-and-avoid-the-open rule), **Nick chose retire.** Push whenever a change is ready — no special-casing market hours by wall-clock alone. The narrow hub_mcp-specific "preferred" note above is unaffected (different rationale — FastMCP restart interrupting a live Olympus pass, not webhook delivery). If live inbound webhook traffic is a known concern in a specific moment (e.g., a burst of signals is actively firing), that's still worth weighing case-by-case — this retirement removes the blanket clock-based rule, not judgment.
...
- **Bulk-checkpoint ban — RATIFIED 2026-07-24 (Nick).** Never make blanket "checkpoint" commits that stage the whole working tree (`git add .` / `git add -A` / `git commit -a`). Every commit is **pathspec-scoped** to the exact files a change touches. This repo is a multi-lane, multi-worktree environment, so the working tree routinely holds other lanes' uncommitted edits and untracked artifacts; a bulk checkpoint sweeps those into an unrelated commit and cross-contaminates lanes (as it did in the 2026-07-24 S-6 build tree). When the shared checkout is tangled and a docs record must land on `main`, commit it from a **clean worktree at `origin/main`**, not through the tangled tree.
  - **Per-file triage before staging.** Review each modified/untracked path individually and decide include/exclude on purpose. Never stage a directory or glob wholesale (`git add <dir>/`, `git add '*.py'`) — enumerate the exact files the change touches.
  - **Credential pattern-scan before `git add`.** Scan the exact paths about to be staged for secret patterns (`postgres://`, `password`, `secret`, `token`, `BEARER`, `API_KEY`, `X-API-Key`). Any nonzero hit blocks the stage pending review — counts only, never open or echo the match. (Written after the 2026-07-19 `reconcile_rh.py` DB credential reached origin via a broad `docs+scripts checkpoint`.)
  - **Tripwire.** If `git status` / `git diff --cached` shows a path the change did not intend to touch, STOP and unstage it — that staged stranger *is* the bulk-checkpoint failure mode, not a harmless extra.
...
The Olympus committee personas (TORO, URSA, PYTHAGORAS, PYTHIA, THALES, DAEDALUS, PIVOT) are authored as Claude.ai skills. Source of truth lives in this repo; the installable artifact is generated by a packaging script and uploaded manually to Claude.ai.

**Folder layout per agent:**

```
skills/<agent>/
├── SKILL.md                    Identity, principles, data checklist, output format, hard rules
└── references/
    ├── equities.md             Equity/options/high-convexity playbook
    └── crypto.md               Crypto playbook (currently stubbed pending Stater Swap rebuild)
```
...
2. Re-run the packager:
   ...
   scripts\package-skill.bat all          # all agents
   ```
3. The packager outputs `dist/skills/<agent>.skill` (a ZIP with the `.skill` extension).
4. Upload the resulting `.skill` file via claude.ai → Customize → Skills → "+" → upload.
