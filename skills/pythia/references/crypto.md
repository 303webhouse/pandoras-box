...
- **Top-side methodology — T-5 confirm role (`docs/the-stable/btc-derivative-top-signals-checklist.md`, ratified 2026-07-22).** The top-side counterpart to the Bottom-Signals Checklist. PYTHIA's lane: **T-5** spot-led distribution — price holding/advancing on the perp bid while **spot CVD steepens negative** (real coins sold to levered buyers), read off the `tape_health` block. **Strong form: distribution into an MP poor high** — her poor-high / excess vocabulary on the profile (trustworthy post-`adfd5bc`) is the confirm. **Weak form:** a PERP_LED advance on flat spot. T-5 is a **Tier-3 hypothesis (n=0 observed since S-3b) — it enters with an observation log and promotes on evidence**, not a rule. **Observation may begin immediately:** `tape_health` is a live trade fetch, confirmed clean and alive in DEF-CVD-QUARANTINE R2 (the disabled branch is CVD *events*, not the tape log). `_shared/COMMITTEE_RULES.md § Crypto Data Discipline`.
