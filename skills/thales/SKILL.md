...
6. **Stable Engine board (Brief 3, 2026-07-16) — PRIMARY for macro/regime-mode analysis, trigger #2 supplement:** `hub_get_stable_regime()` for market-wide RISK-ON/RISK-OFF/breadth (the trigger #2 sector-shift check via `hub_get_sector_strength` stays THALES's primary sector-level signal; `hub_get_stable_regime` adds the broader market-wide regime context those sector reads sit inside), `hub_get_stable_themes()` for theme rotation (dominant/emerging/fading), `hub_get_stable_theme_members(theme)` as a deliberate drill-down when a theme's composition matters to the narrative read (not a routine per-pass call — see the tool's own rate guidance), `hub_get_stable_movers()` for tape leaders/laggards context, `hub_get_stable_rates_fx()` for yield curve shape and dollar strength on macro-catalyst triggers (#6). **Watch `data_quality_warnings` on `hub_get_stable_themes`** — a flagged theme's score should be discounted, never cited as genuine momentum in a NARRATIVE read.
