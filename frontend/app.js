...
document.addEventListener('DOMContentLoaded', () => {
    // RADAR watchlist section removed (P0 declutter 2026-07-02). initWatchlist /
    // initWatchlistV3 and their poller (watchlistRefreshInterval) are no longer
...
// HYDRA squeeze panel removed (P0 declutter 2026-07-02). Only the exposure
// warning survives — relocated to #positions-exposure-warning at the top of the
// positions panel. The scores/convergence pollers, panel tabs, refresh button,
// and their render paths are gone. Lightning cards (below) are independent —
// they render into #tradeSignals and are unaffected.
function initHydra() {
    fetchHydraExposure();
    managedInterval(fetchHydraExposure, 300000);
}
...
async function fetchHydraExposure() {
    try {
        ...
        const warningEl = document.getElementById('positions-exposure-warning');
        if (!warningEl) return;
        if (data.warning) {
