...
async def init_database():
    ...
    async with pool.acquire() as conn:
        ...
        # position twice. Mirrors migrations/039; the expansion itself lives in
        # database/legs_migration.py and runs below, guarded by its own outcome table.
        await conn.execute("""
        ...
        """)
        try:
            from database.legs_migration import run as _expand_legs
            _legs = await _expand_legs(conn)
            if _legs:
                print(f"  position legs migration: {_legs}")
        except Exception as e:
        ...
        # the caller chose, and this is the half that can be checked. Mirrors migrations/038.
        await conn.execute("""
