...
async def send_uw_screenshot_request(time_slot: str) -> None:
    ...
    messages = {
        "morning": (
            "📊 **Morning data request** — when you can, please drop screenshots of:\n"
            "• **UW Market Tide**\n"
            "• **UW Dark Pool**\n"
            "No rush — anytime before noon works."
        ),
        "afternoon": (
            "📊 **Power hour setup** — if you can grab:\n"
            "• **UW Market Tide**\n"
            "• **UW Dark Pool**\n"
            "This helps prep for late-session signal quality."
        ),
        "close": (
            "📊 **EOD brief data request** — please drop when possible:\n"
            "• **UW Market Tide**\n"
            "• **UW Dark Pool**\n"
            "• **UW Options GEX**\n"
...
def _format_factor_health_context(composite_payload: Optional[Dict[str, Any]]) -> str:
    ...
    if stale_count > 5:
        summary += f"\n⚠️ Low data confidence - {stale_count} factors stale. Composite bias may be unreliable."
    return summary
...
def _format_convergence_context(payload: Optional[Dict[str, Any]]) -> str:
    ...
    for event in events[:3]:
        ...
        lines.append(f"- 🎯 CONVERGENCE: {ticker} {direction} — confirmed by {source_names} ({strength})")
    return "\n".join(lines)
...
def _is_csv_attachment(attachment: discord.Attachment) -> bool:
    ...
    # Accept .csv, .txt (Robinhood exports use .txt), or csv content-type
    return name.endswith(".csv") or name.endswith(".txt") or "csv" in ctype
