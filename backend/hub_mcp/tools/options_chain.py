...
DESCRIPTION = (
    ...
    "implied volatility, and Black-Scholes Greeks (delta, gamma, theta, vega). "
    "Plus chain-level aggregates: spot, IV rank (0-100), max pain (filtered "
    "to the requested expiry), total call/put open interest, greeks_source, "
    "and the UW server timestamp + source flag.\n\n"
    "Greeks are Black-Scholes modeled (greeks_source='bs_computed'), computed "
    "hub-side from UW-provided implied_volatility. They are NOT market-observed "
    "Greeks from a broker feed. BS assumes European exercise and zero dividends "
    "— modeled Greeks typically differ from broker-displayed Greeks by <5% on "
    "ATM contracts; divergence is larger for deep-ITM puts on dividend-paying "
    "underlyings. For execution-critical sizing on large positions, cross-check "
    "against your broker's Greeks. Contracts with null IV will have null Greeks.\n\n"
    ...
    "strike picking, liquidity check, IV-regime read, delta-based sizing). "
    "Also call when the user asks about \"the chain,\" "
    "\"what's IV rank,\" \"where's max pain,\" \"what's delta,\" "
    "or any equivalent.\n\n"
    ...
    "call this for historical option pricing.\n\n"
