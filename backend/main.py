...
try:
    ...
    # ─── Brief D rev2 Phase 0 Investigation 2 — TEMPORARY repro endpoint ───
    # Runs the exact rev2 OAuth-storage chain (Redis + RedisStore + Fernet)
    # against REDIS_URL inside the live Railway container and returns JSON.
    # Container-local TLS trust + container-to-Upstash network path are the
    # rev2 environment; reproducing locally via `railway run` does not work
    # on Windows due to a Python CA-bundle mismatch (verified empirically
    # 2026-05-25). Faithful repro requires running here.
    #
    # Gated by PHASE0_REPRO_TOKEN env var (must match ?token=). Returns 503
    # when token env var is unset (i.e. when feature is disabled). Returns
    # 403 when token does not match. Only emits exception class names — no
    # URL, no values, no key material. Removed entirely once Phase 0 lands.
    @app.get("/mcp/v1/_repro", include_in_schema=False)
    async def _mcp_v1_repro(token: str = ""):
        import hmac
        from fastapi.responses import JSONResponse

        expected = os.environ.get("PHASE0_REPRO_TOKEN")
        if not expected:
        ...
        if not redis_url:
            return JSONResponse(
                {"status": "error", "error": "REDIS_URL not set in container env"},
                status_code=500,
            )
        ...
        store = RedisStore(client=client, default_collection="phase0-repro")
        fernet = Fernet(Fernet.generate_key())
        chain = FernetEncryptionWrapper(
            store, fernet=fernet, raise_on_decryption_error=False
        )
        sentinel = {"foo": "bar"}

        async def _put_get(key):
            try:
                await chain.put(key=key, value=sentinel)
            except Exception as exc:
        ...
        results = []
        try:
            # S1: immediate put-then-get (the rev2 case)
            t0 = _time.monotonic()
            ok, err = await _put_get("phase0-s1")
            results.append({
                "name": "immediate",
                "ok": ok,
            ...
            # S3: two concurrent put-get pairs via asyncio.gather
            t0 = _time.monotonic()
            try:
                ...
                ok3 = all(p[0] for p in pair)
                errs3 = "; ".join(p[1] for p in pair if not p[0]) or None
            except Exception as exc:
                ok3 = False
                errs3 = f"gather raised {type(exc).__name__}"
            results.append({
                "name": "concurrent",
                "ok": ok3,
                "ms": round((_time.monotonic() - t0) * 1000, 1),
    ...
    logger.info("✅ /mcp/v1/_repro endpoint registered (Phase 0 diagnostic, gated)")
