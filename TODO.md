...
- [ ] **Backtest — Phase 0.5 Forward-Logger Cron — DEPLOY WITHIN 48 HOURS.** New scope item. Daily VPS cron at 21:00 UTC pulls flow-alerts / darkpool (cursor-paginated) / net-prem-ticks / spot-exposures / daily greek-exposure for watchlist (~10-20 tickers, start small) and writes to `data/cache/uw/{data_type}/{symbol}/{YYYYMM}.parquet`. **Every day of delay = one day of proprietary history permanently lost.** Needs rate-limit burst-test as prerequisite. **CC brief ready at `docs/codex-briefs/brief-phase-0-5-uw-forward-logger.md` — NEXT CC TASK.**
