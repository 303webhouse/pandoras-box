...
- PYTHIA (Market Profile) added as 5th agent between TECHNICALS and PIVOT
...
from committee_prompts import (
    ...
    TECHNICALS_SYSTEM_PROMPT, PYTHIA_SYSTEM_PROMPT, PIVOT_SYSTEM_PROMPT,
)
...
# ── Committee (5 agents: TORO, URSA, TECHNICALS, PYTHIA, PIVOT) ──
...
def run_committee(signal: dict, context: dict, api_key: str, technical_data: dict | None = None) -> dict:
    ...
    try:
        ...
        mp_ticker = signal.get("ticker", "")
        api_url = context.get("api_url") or os.environ.get("PANDORA_API_URL") or ""
        api_key_val = context.get("api_key") or os.environ.get("PIVOT_API_KEY") or ""
        if mp_ticker and api_url:
            mp_block = build_mp_levels_context(mp_ticker, api_url, api_key_val)
            if mp_block:
                base_context = base_context + "\n\n" + mp_block
        # Also inject SPY MP levels for macro context (unless ticker is already SPY)
        if mp_ticker.upper() != "SPY" and api_url:
    ...
    # ── PYTHIA (Market Profile) ──
    log.info("Calling PYTHIA agent...")
    pythia_raw = call_agent(
        system_prompt=PYTHIA_SYSTEM_PROMPT,
        user_message=_agent_context("PYTHIA"),
        api_key=api_key,
        max_tokens=500,
        temperature=0.3,
        agent_name="PYTHIA",
    ...
    if pythia_raw:
        pythia_response = parse_analyst_response(pythia_raw, "PYTHIA")
    else:
        pythia_response = {
            "agent": "PYTHIA",
            "analysis": "[ANALYSIS UNAVAILABLE — PYTHIA agent timed out]",
            "conviction": "MEDIUM",
        }
    ...
    if pivot_raw:
        ...
        pivot_response = {
            ...
            "synthesis": (
                ...
                f"Structure: {pythia_response['analysis']}"
            ),
            "conviction": "LOW",
            "action": "WATCHING",
