...
def post_discord_embed(token: str, channel_id: str, embed: dict[str, Any]) -> dict[str, Any]:
...
def fetch_signals(api_url: str, api_key: str) -> list[dict[str, Any]]:
    ...
    # Primary endpoint is /signals/active. If it times out, fallback to queue.
...
def run(channel_id: str, min_score: float, dry_run: bool) -> dict[str, Any]:
    ...
    for signal in signals:
        ...
        if dry_run:
            print(json.dumps({"dry_run_embed": embed}, ensure_ascii=False))
        else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Pivot II trade idea poller")
