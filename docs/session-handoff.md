...
- [2026-02-20 16:57:30 UTC] Incident triage: Railway Postgres entered recovery loop after WAL write failure (`No space left on device` on `pg_wal/xlogtemp.*`) during heavy `price_history` upserts. No repository code changes were made in this session; recommended immediate action is storage expansion and restore/recovery from latest healthy backup if startup cannot complete.
- [2026-02-20 17:08:00 UTC] Mitigation patch for Railway Postgres volume incidents: updated `backend/analytics/price_collector.py` to (1) skip no-op conflict updates via `IS DISTINCT FROM`, (2) tighten non-backfill fetch windows, (3) restrict auto-collected tickers to watchlist + recent signals with caps, and (4) add env-tunable guardrails for DB-size thresholds and collector scope; updated `backend/database/postgres_client.py` with `idx_price_timeframe_timestamp`; documented new env knobs and disk-full runbook line in `QUICK_START.md`. Validation: `python -m compileall backend/analytics/price_collector.py backend/database/postgres_client.py` and `python -m pytest backend/tests -q` (20 passed).
- [2026-02-20 17:12:00 UTC] Data-retention review: confirmed no automated cold-storage archive pipeline exists for `price_history` (current controls are in-DB retention trim and manual CSV export endpoints). Recommended next step is tiered storage (Postgres hot window + object-storage parquet archive + purge-after-verify job) to control Railway cost growth.
- [2026-02-20 17:16:00 UTC] Added local cold-archive workflow for `price_history`: new CLI `backend/jobs/archive_price_history.py` exports old rows to compressed batch CSVs + run manifest on local disk, with optional `--purge` deletion after export; updated `QUICK_START.md` with runbook commands, `config/.env.example` with archive/guardrail vars, and `.gitignore` to ignore `data/archives/` outputs. Validation: `python -m backend.jobs.archive_price_history --help`, `python -m compileall ...`, and `python -m pytest backend/tests -q` (20 passed).
- [2026-02-20 17:20:00 UTC] Automated local archive scheduling: added `scripts/run_price_history_archive.ps1` (lockfile + logging + env-driven defaults + optional purge/dry-run) and `scripts/register_price_history_archive_task.ps1`; registered Windows Task Scheduler job `\TradingHub-PriceHistoryArchive` (daily at 02:15 local) to run archive hands-off. Updated `QUICK_START.md` with install/run/query commands and `config/.env.example` with `PRICE_HISTORY_ARCHIVE_PURGE`.
- [2026-02-20 17:24:00 UTC] Auto-archive scheduler finalized: registered `\TradingHub-PriceHistoryArchive` via `scripts/register_price_history_archive_task.ps1` (daily 02:15 local, status=Ready, action=`powershell.exe ... scripts/run_price_history_archive.ps1`). Attempted ONLOGON secondary trigger but Windows returned `Access is denied` in this user context, so retained daily-task-only configuration for reliability.
- [2026-02-20 19:41:30 UTC] Archive status check: scheduled task `\TradingHub-PriceHistoryArchive` has not executed yet (`Last Run Time` shows 1999 baseline). Manual dry-run command `python -m backend.jobs.archive_price_history --older-than-days 2 --dry-run --max-batches 1` succeeded against remote DB with `candidate_rows=610656`, generating manifest at `data/archives/price_history/run_20260220T194026Z/manifest.json`. Public backend health endpoint still returns `502 Bad Gateway`.
- [2026-02-20 19:45:00 UTC] Executed full local archive + purge for `price_history`: `python -m backend.jobs.archive_price_history --cutoff 2100-01-01T00:00:00Z --purge`. Result: `candidate_rows=614546`, `rows_exported=614546`, `rows_deleted=614546` across 25 compressed batches in `data/archives/price_history/run_20260220T194342Z` with manifest. Post-check dry-run confirms `candidate_rows=0` (table effectively emptied). Public API health endpoint still returns `502 Bad Gateway`, so backend service likely needs Railway restart/redeploy despite DB connectivity from direct archive job.
- [2026-02-20 19:52:37 UTC] Railway outage triage: current failure is persistent No space left on device during WAL recovery (pg_wal/xlogtemp.*) on Railway Postgres. Verified local config/.env DB target is Supabase (DB_HOST=aws-1-us-east-2.pooler.supabase.com), so prior local archive/purge did not act on Railway volume. Next required action is Railway volume expansion or restore from Railway backup, then immediate write-throttle/purge policy on Railway DB.
- [2026-02-20 19:54:08 UTC] Added incident kill-switches for price history in backend/scheduler/bias_scheduler.py: ENABLE_PRICE_HISTORY_COLLECTION (default true) and ENABLE_PRICE_HISTORY_BACKFILL (default false). Scheduler now skips startup backfill unless explicitly enabled and can disable 5-min collection entirely. Updated config/.env.example and QUICK_START.md; compile check passed.
...
- [2026-02-23 01:26:16] Answered PnL query from latest Account Updates CSV (3509261b-7c80-54a0-aab9-0220e4b2be96.csv): realized closed-trade PnL +$118.94 through 2026-02-18; 46 positions remain open (unrealized not included).
- [2026-02-23 01:28:14] Verified trade-upload status for requested 2026-01-01 to 2026-02-19 range: DB has 1 trade row (SPY on 2026-02-18), so Jan-Feb history is not fully uploaded. Latest CSV available spans 2026-01-02 to 2026-02-18.
- [2026-02-23 01:36:38] Imported latest Robinhood CSV (3509261b-7c80-54a0-aab9-0220e4b2be96.csv) into analytics trades: reconciled parsed 133 trades to DB with missing_by_strict_key=0 (DB total 134 due 1 pre-existing extra row), closed PnL in range now +$118.94. Patched backend/analytics/queries.py date-window SQL in trade_exists_duplicate/find_matching_signals and backend/analytics/api.py to parse expiry fields as date before insert.

## 2026-02-26 (Committee Expert Review + Tier 1 Build)

- Ran 3 parallel Opus expert review agents (TA Expert, Buy-Side Analyst, Sell-Side Derivatives Strategist) against committee training system, prompts, parsers, and Stable education files.
- All 3 converged on same critical gap: agents are well-instructed on options analysis but data-starved (no IV, no vol regime, no economic calendar, no structure recommendations).
- Produced 20-item roadmap across 3 tiers + 6 bug fixes, saved to `docs/committee-review-recommendations.md`.
- Implemented full Tier 1 (4 critical gaps + 6 bug fixes):
  - **HV20 + Vol Percentile** in `committee_context.py`: 20-day annualized HV as IV proxy, percentile rank vs 1-year range, 4-tier vol regime labels, trend direction. ATR fixed to Wilder smoothing, MACD lookback expanded 4→7 bars.
  - **Economic Calendar** (`data/econ_calendar_2026.json` + `committee_context.py`): 66 events (FOMC/CPI/NFP/PCE/GDP/OPEX Mar-Dec 2026), `fetch_economic_calendar()` filters by DTE window, `format_economic_calendar()` renders context block.
  - **DTE-aware Earnings** in `pivot2_committee.py`: `check_earnings_proximity(ticker, dte_days=30)` uses `max(14, dte_days)` window, added `within_dte_window` field. Econ calendar wired into `run_committee()`.
  - **STRUCTURE/LEVELS** in `committee_prompts.py` + `committee_parsers.py`: PIVOT now outputs options structure (credit/debit based on vol percentile) and entry/stop/target/R:R levels. Added to known_prefixes and Discord embed.
  - **Quick fixes**: Token limits TECHNICALS 500→750, PIVOT 1000→1500. OI 500→2000. Bid-ask absolute→% of mid. SMA/EMA clarity notes in TORO + TECHNICALS.
- Deployed all 5 files to VPS via SCP, syntax verified, live tested: SPY showing HV 12.8% at 15th percentile, 7 upcoming economic events. Restarted OpenClaw.
- **Next:** Tier 2 build (RSI/MACD divergences, SMAs, portfolio risk, position sizing).
- Implemented full Tier 2 (5 items):
  - **SMAs + CTA Zone** in `committee_context.py`: SMA 20/50/120/200 computed from daily close. CTA zone classification (GREEN/YELLOW/RED/GREY) based on price-SMA alignment. Formatted as separate section below EMAs.
  - **RSI/MACD Divergence Detection** in `committee_context.py`: Swing pivot analysis (3-bar window) over last 30 bars. Detects regular bullish/bearish divergences for both RSI and MACD histogram. Flags shown inline with indicator values (e.g., `RSI(14): 68.2 [overbought] **DIVERGENCE: bearish**`).
  - **Portfolio Risk Context** in `committee_context.py` + `pivot2_committee.py`: `fetch_portfolio_context()` calls Railway API for balances + positions. `format_portfolio_context()` renders account balance, open positions, capital at risk %. Wired into `build_market_context()` and injected into `run_committee()`. Live test: $4,469 balance, 2 positions, 2.3% at risk.
  - **SIZE field** in `committee_prompts.py` + `committee_parsers.py` + `pivot2_committee.py`: PIVOT now outputs position sizing (HIGH=3-5%, MEDIUM=1.5-2.5%, LOW=watching). Added to parser, Discord embed.
  - **Realized Vol text**: MACD histogram value surfaced, vol regime text updated with IV proxy language.
- All 4 files deployed to VPS, syntax verified, live tested (SPY: SMAs + CTA GREEN zone, portfolio context working), OpenClaw restarted.
- Implemented full Tier 3 (7 tasks + 1 bug fix, 3 items skipped):
  - **Bollinger Bands + Squeeze** in `committee_context.py`: BB(20,2) upper/lower/width, squeeze detection (current bandwidth below 20th percentile of 90-day history). Formatted as `BB(20,2): $199.13 / $175.03 (width: 12.32%) **SQUEEZE — breakout imminent**`.
  - **Volume Trend** in `committee_context.py`: Up-volume vs down-volume 10-day rolling ratio. Labels: >1.2 accumulation, <0.8 distribution, else neutral. Shown inline with ATR/volume line.
  - **5-day Rolling VWAP** in `committee_context.py`: `(typical_price * volume).rolling(5).sum() / volume.rolling(5).sum()`. Shown as `5d VWAP: $191.49 (price above)`.
  - **Relative Strength vs SPY** in `committee_context.py`: 20-day return of ticker minus 20-day return of SPY. Required yfinance MultiIndex column fix (`if hasattr(spy_close, "columns"): spy_close = spy_close.iloc[:, 0]`). Skipped for SPY itself.
  - **P&L State Injection** in `committee_context.py` + `pivot2_committee.py`: `fetch_recent_pnl_context()` reads `outcome_log.jsonl`, counts consecutive losses from most recent, returns formatted block with playbook warning if 2+ consecutive losses. Wired into `run_committee()` after portfolio block.
  - **Pin Risk** in `committee_prompts.py`: Added to URSA "Options-Specific Risks" section — flags credit spreads <7 DTE for early assignment risk on American-style options.
  - **0DTE Gamma Awareness** in `committee_prompts.py`: Added section 8 to TECHNICALS prompt — 0DTE microstructure effects (pre-noon mean-reversion, post-2pm trend acceleration, OPEX week amplification).
  - **MEDIUM Conviction Bug Fix** in `committee_analytics.py`: Changed 4 locations where MEDIUM/WATCHING was counted as "correct" to pass (no accuracy credit). Prevents accuracy inflation.
