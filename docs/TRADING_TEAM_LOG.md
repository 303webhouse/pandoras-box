...
### 2026-02-25 — Brief 10: Position Tracking Gap Fixes + IBKR Activation + yfinance Pricer
**Agent:** Claude Code (Sonnet 4.6)
**What happened:** Implemented all 7 gaps from Brief 10 plus the follow-on yfinance pricer task. Railway API updated and auto-deployed: `open_positions` gained `signal_id` + `account` columns (ALTER TABLE migrations, idempotent), `closed_positions` table created with full P&L schema, `POST /positions/sync` now supports `partial: true` (screenshot safe) + per-account filtering, `POST /positions` single create endpoint added with 409 duplicate guard, close endpoint rewritten to write `closed_positions` rows with computed pnl_dollars/pnl_percent/hold_days. VPS scripts added to repo: `ibkr_poller.py` (full IBKR poller, sends `partial: false, account: ibkr`), `committee_interaction_handler.py` (TAKE button saves `last_take.json` + prompts for fill screenshot), `add_cron_jobs.py` helper (adds IBKR + yfinance cron jobs to OpenClaw config in UUID structured format), `yfinance_price_updater.py` (prices non-IBKR positions every 15 min via yfinance). Railway health confirmed healthy; all 9 existing positions correctly return `signal_id: null, account: "robinhood"` proving migration ran.
**Files changed:** `backend/api/portfolio.py` (full rewrite — Gaps 1-4), `backend/database/postgres_client.py` (ALTER TABLE + CREATE TABLE closed_positions + indexes), `docs/pivot-knowledge/RH_SCREENSHOT_RULES.md` (partial sync rules, last_take.json linkage, single fill format), `openclaw/scripts/ibkr_poller.py` (new), `openclaw/scripts/committee_interaction_handler.py` (new with Brief 10 TAKE prompt), `openclaw/scripts/yfinance_price_updater.py` (new), `openclaw/scripts/add_cron_jobs.py` (new — VPS cron helper), `openclaw/cron/jobs.json` (added ibkr-position-poller, ibkr-quotes-poller, yfinance-price-updater), `openclaw/README.md` (new)
**Deviations from brief:** SSH to VPS not available from sandbox — VPS cron jobs and script deployment captured in `openclaw/` directory in repo with `add_cron_jobs.py` helper to run on VPS manually. The helper reads the existing OpenClaw jobs.json format and adds jobs with correct UUID structure. Run `python3 /opt/openclaw/workspace/scripts/add_cron_jobs.py && systemctl restart openclaw` on VPS to activate. `ibkr_quotes.py` is a pre-existing VPS file (from Brief 07) — not recreated here.
**Next blocker:** Deploy `openclaw/scripts/` to VPS (`rsync openclaw/scripts/ root@188.245.250.2:/opt/openclaw/workspace/scripts/`), run `add_cron_jobs.py`, restart openclaw. Also: DST adjustment needed March 9 2026 — change IBKR and yfinance cron schedules from `14-21` to `13-20` UTC.
