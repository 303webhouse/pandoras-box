...
6. **Cost is stored at print time, not reconstructed later (R-IV.303(d), from the second market read).** Every sink stores the **underlying's bid/ask as of the print**. **Without it every excess figure is costless BY CONSTRUCTION** — not merely uncorrected, but structurally incapable of ever being corrected, because the spread that existed at the moment of the print is **not recoverable from any later source**. A cost model applied afterwards estimates a distribution; it cannot recover the number. This is the one item on this list that **cannot be retrofitted**: the other five can be added to a running collector, and this one silently loses its evidence every cycle it is missing.
