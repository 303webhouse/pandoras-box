...
LAYER 4: NOTIFICATION + MANUAL COMMITTEE REVIEW
├── Candidates scoring 80+ → Discord #auto-scout notification
│   ├── Include: ticker, direction, score, signal type
│   ├── Include: key UW flow data (P/C, premium, sentiment)
│   ├── Include: GEX regime, dark pool sentiment
│   └── Include: entry/stop/target levels from scanner
├── Nick sees notification on phone/desktop
├── Opens Claude (chat, Cowork, or app)
├── Requests full committee review with PhD-level analysis
│   ├── Claude has: memory, open positions file, conversation history
│   ├── Claude has: all Layer 2 enrichment data via hub API
│   └── Quality: full Claude reasoning >> budget Haiku API calls
├── ELIMINATES: OpenRouter API costs, pivot2-interactions service,
│   committee_prompts_v2.py, and the entire VPS committee pipeline
└── Sub-80 signals → logged to DB for backtest analysis
...
The architecture is sound. Three free/existing layers (TV webhooks, uw-watcher replacement via API, TV MCP) handle real-time signals. UW API Basic handles enrichment at 8% of its daily budget. Committee reviews happen manually in Claude at full quality — dramatically better than the automated Haiku pipeline that failed twice today. This is a plumbing job, not a new build.

URSA's right about signal overload — that's the failure mode to design against. The filtering layer is where the edge lives or dies. A Phase 2G that posts 50 notifications a day is worse than no Phase 2G. A Phase 2G that surfaces 2-3 A-setup candidates a day, which Nick then runs through a full Claude committee review, is a game-changer.
...
4. Auto-scout scoring + filtering layer
5. Discord notification output for high-score candidates
...
**Goal:** Autonomous flow screening → scored candidates → Discord notification
...
- [ ] Gate: score ≥ 80 → post to Discord
- [ ] Gate: max 10 candidates/day, max 3 per sector
- [ ] Post scored candidates to Discord #auto-scout channel with summary
- [ ] Include: ticker, direction, score breakdown, key UW flow data, GEX levels
- [ ] Nick reviews notification → opens Claude → requests full committee review
- [ ] Committee review runs at full Claude quality with memory + positions context
- [ ] Performance tracking: log every signal with outcome

**Definition of done:** Phase 2G is live and posting 2-5 auto-scout notifications per day to Discord. Nick requests manual committee reviews in Claude for candidates that interest him.
