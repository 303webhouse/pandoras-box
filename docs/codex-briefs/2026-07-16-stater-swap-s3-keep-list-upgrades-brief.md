...
## §12 — Amendment FA-7 (Fable ruling, 2026-07-16 post-Phase-0) — resolves the blocking finding

**Ruling: Option 1 — parametrize the four client functions.** Rationale: Addendum A-5 ("full two-column dial for BTC/ETH") is a ratified committee requirement, not a preference — Option 2 formally defers it; Option 3 half-defers it while saving almost nothing (OI lives in the same file and pattern as funding; basis is the only genuinely extra work, and OKX carries ETH dated futures). The capability matrix's own Cross-cutting note #1 named this exact parametrization the R-2/R-3 prerequisite — the bill has come due inside R-2, and paying it now is settled scope, not creep. Zero new vendors, $0 spend: this changes code shape, not data scope. Nick may veto this ruling with one word before Phase 1 code lands; absent a veto, CC proceeds on FA-7.

**Binding implementation contract — inserts as Phase 1.5 (between §3 and §4; all other phase numbering unchanged):**

1. Parametrize `get_funding_rate()`, `get_open_interest()` (`coinalyze_client.py`), `get_25_delta_skew()` (`deribit_client.py`), and `get_quarterly_basis()` (`binance_client.py`) to accept a symbol argument **defaulting to `"BTC"`** — every existing caller (the 5-min `btc_bottom_signals.py` loop, the market-structure filter) is signature-compatible and behavior-identical by construction.
2. **Per-symbol cache keys — mandatory, not stylistic.** The current module-global keys Phase 0 found (`quarterly_basis`, `skew_25d`, etc.) would cross-poison symbols the moment a second symbol calls in: an ETH read must never evict or masquerade as BTC's cached value, and vice versa.
3. Symbol → vendor-symbol maps sourced from `backend/config/crypto_symbol_matrix.py` (Coinalyze `.A` aggregates, Deribit currencies, OKX instrument ids) — never inline literals in client code.
4. Guards codified as the §4.2 vocabulary: Deribit instrument-count > 0 before trusting currency presence (the SOL trap, matrix note 4); vendor-level absence → `NA` with a stated reason (`NO_DERIBIT_COVERAGE`, `NO_DATED_MARKET`, `NOT_LISTED_BINANCE_SPOT`) — **never a mislabeled BTC value, never zeros.** The fake-healthy failure Phase 0 named (BTC's funding returned as "ETH's") must be structurally impossible post-FA-7.
5. **BTC regression gate:** capture one live BTC read per parametrized function *pre-change*; post-change BTC reads must match in shape and sanity. The legacy 5-min bottom-signals loop and the market-structure filter must not be able to tell this refactor happened. Evidence in the findings doc.
6. Per-symbol `crypto_vendor_health` + sanity-bounds wiring on every parametrized path (S-1 patterns, already per-client).

7. **Scope check on the other five capitulation inputs** (closing CC's own follow-up note in 1.1): liquidations + term structure = Coinalyze — same file, same parametrization pattern, **include them in this pass** (the dial is per-symbol; leaving two Coinalyze reads BTC-locked would recreate the same contradiction one column over); spot orderbook = Binance-spot — parametrize, with HYPE/FARTCOIN honest-`NA:NOT_LISTED_BINANCE_SPOT` (matrix: HTTP 400, not listed); stablecoin APRs = market-wide by design — uniform across symbols, client untouched; VIX = macro-uniform — untouched (its yfinance read is pre-existing macro context, not a crypto hot path; logged, not fixed here).
8. **OKX dated-futures discovery for basis is per-symbol and evidence-recorded** in the findings doc; symbols without a dated market → `NA:NO_DATED_MARKET`. A-5's own text blesses N/A outside coverage — BTC/ETH are the must-haves, and both carry OKX dated markets.
9. **Done Definition addendum — Done-16:** parametrized per-symbol reads proven live from Railway (Coinalyze funding/OI/liquidations/term-structure across the six; Deribit ETH real + SOL honest-unavailable via the instrument-count guard; orderbook + basis exactly per discovery), the BTC regression gate green, and per-symbol cache isolation proven (an ETH read demonstrably does not evict or alter BTC's cached value).

**Consequential notes:** Done-7's "BTC/ETH full columns" now has a real mechanical path — §4.3's "computations unchanged" stands for the BTC legacy loop, while the cycle engine reaches non-BTC symbols *through* Phase 1.5 rather than through an assumption. Finding 1.9's guidance is adopted verbatim for §6.3: `hub_get_crypto_market_profile` follows `hub_get_crypto_quote`'s no-guard pattern (dedicated crypto tool; the disambiguation guard correctly lives only in the shared `hub_get_quote` entry point). Finding 1.6's four-list correction is adopted: §3.4's prune applies to `bias_scheduler.py:3493`'s list **only**; the `tradingview.py` classification set is explicitly out of scope.

*FA-7 recorded in the coordination lane, 2026-07-16 16:1x MDT, per the S-1 cutover-ruling precedent (Fable rules, Nick holds the veto). CC: resume at Phase 1, execute Phase 1.5 before §4.*
