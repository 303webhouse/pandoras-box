...
### 9.6 First-RTH verification fixes (2026-07-02 — §5 pass on the first live RTH window)

The first real RTH session exposed three defects the shadow feed would have carried silently — all found + fixed same-day, feed confirmed healthy (69+ rows, 0 dups):

- **BUG#1 — INSERT DataError (blocking; 0 rows):** `created_at` (UW timestamp **string**) bound to `$2::timestamptz` → asyncpg's timestamptz codec expects a `datetime`, raised `DataError` on **every** row. Poller was running (governor showed 120+ calls) and fail-open swallowed the per-row errors, so it looked alive while persisting nothing — a fake-healthy trap. Fix: `$2::text::timestamptz` (asyncpg sends text, PG casts), mirroring the prior `$1::date`→`$1::text::date` fix. `$15::jsonb` confirmed proven-safe (used codebase-wide). Commit `3e95793`.
- **BUG#2 — `bias_level_at_fire` always NULL:** `utils/bias_snapshot.get_bias_snapshot()` copied `gex_regime` out of the composite but never `bias_level`, so the poller's `snap.get("bias_level")` was always None (gex populated, bias didn't). Additive 1-line fix exposing `bias_level` top-level (CompositeResult carries it). Commit `c0826ef`. Pre-fix rows keep NULL (no backfill; bias-at-fire is context, not a mandatory §6 split).
- **BUG#3 — prior_5d cache per-tick → ~3× over quota:** governor showed `triton_flow_shadow` pacing to ~1,400 calls/day vs the **450** claimed in §9.2, because `prior5_cache` was rebuilt each 120s tick and recurring tickers (MU etc.) re-fetched bars every tick. Made the cache **per (ticker, day)** across ticks (module dict, pruned on rollover) → ~345 calls/day, so 450 actually holds at enforce. Observe mode meant zero functional impact today. Commit `511f5c4`.

**Verified sound (not bugs):** `spot_at_fire` values that looked inflated (MU ~$1050, BE ~$298) are **real** — matched live `hub_get_quote` (MU 1049.33 / BE 306.68 / NVDA 199.13) and UW's own strikes cluster there. Flow-alerts `underlying_price` and the grader's `get_ohlc` closes share one UW basis, so ratio-based forward returns grade correctly. Dedup/idempotency proven (rows == distinct `uw_alert_id`). **Grader sign-check deferred to ≥7/6:** T+1 requires an `'r'` session after 7/2, and 7/3 is the July-4th holiday, so the first gradeable close is Mon 7/6.
