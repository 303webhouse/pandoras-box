...
**Update 2026-07-21** (per Nick+Fable, backlog-only re-flag during DEF-SIGNAL-METADATA): `Crypto Scanner` is **still fully silent — now 18 days** (last signal 2026-07-03, re-confirmed by live query 2026-07-21). Nothing has alerted in that entire window, which is exactly the blind spot this item proposes to close; the dormancy remains attributed to the benign root cause above (bearish structure + long-only logic), but note that attribution is now 6 days stale and has not been re-validated against current market structure. Re-checking whether the CTA-zone condition still holds should be step 1 of any work here — if the zones have turned and the scanner is *still* silent, the benign explanation no longer covers it and this becomes a real defect rather than a visibility gap.
...
### 9. `confluence_validation.py` — stale `source` vocabulary, tool has never worked
**Bucket:** Tactical / analytics correctness. **Added 2026-07-21** per Nick+Fable ruling (backlog only, explicitly not actioned), surfaced by DEF-SIGNAL-METADATA's adversarial verify pass.
**Why:** `analytics/confluence_validation.py::compute_shadow_validation()` compares server-side-scanner signals against TradingView-webhook signals for the same strategies (Holy_Grail/Scout) to measure overlap. It selects the server side with `source IN ('holy_grail_scanner','scout_scanner','SCANNER')` (line 73) — **none of which any writer has ever emitted.** The real value passed by `holy_grail_scanner.py:353` / `scout_sniper_scanner.py` / `sell_the_rip_scanner.py:655` is `'server_scanner'`. So `server_rows` has matched **zero rows for the life of the tool**, pinning `matches`/`overlap_pct` at 0 and making its verdict (`PASS`/`WAITING`/`NEEDS_TUNING`) meaningless — it can never reach `PASS` by construction. This was masked until now because `signals.source` was itself constant `'tradingview'` (the DEF-SIGNAL-METADATA defect); now that `source` carries real values post-cutline `e8ed614`, the tool *could* be made functional.
**Proposal:** Align the query literals to the real vocabulary (`'server_scanner'`) and scope the analysis to `id >= 15678` (the provenance cutline — pre-cutline rows have no usable `source`). **Caveat that must be designed around:** `'server_scanner'` is shared by three scanners, and `Holy_Grail`/`Scout Sniper` are dual-origin (server + webhook emit the identical `strategy` string), so the pair `(strategy, source)` is required to separate the two sides — see `docs/reference/signal-provenance.md`.
**Gates:** None blocking. Note the tool needs ≥1 post-cutline collection window before it can produce a real verdict regardless of the fix.
