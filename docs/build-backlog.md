...
- **OAuth state persistence on `backend/hub_mcp/`** *(scope sharpened 2026-05-24 via Task 0 + rev1 + rev2 cycles).* OAuth itself is already live (GitHub OAuthProxy via FastMCP, allowlisted; see `docs/codex-briefs/phase-c.1-task0-findings-2026-05-23.md`). The actual problem is that OAuthProxy's state (JTI mappings, DCR client registry, refresh tokens) lives in a Railway-ephemeral file store and gets wiped on each redeploy — hence the manual Claude Desktop reconnect cycle. Two attempts (rev1 + rev2) reverted same-day with documented failure modes. **rev3 is now backlogged as investigation-first, NOT code-first** per closure notes. Investigation list in `docs/strategy-reviews/phase-c.1-rev2-closure-note-2026-05-24.md` §5: (1) Railway worker count, (2) standalone Python reproducer outside FastMCP, (3) OAuthProxy /register vs /authorize source path. Only write rev3 code AFTER investigation confirms the root cause. Bundle this with the Phase C work or split out — bundle-decision deferred to rev3 brief authoring.
