...
- **H-13: RESOLVED (R-IV.527, HELIOS pass 1).** `opacity` dimming is gone from v2: `.rv-item.shadow`, `.k-card.shadow`, `.rv-item.action.acked` and `.k-card.acked .clock-chip` now stay at full opacity. Shadow is a dashed **neutral** (`--text-2`) left rule plus the mono `.shadow-tag` (River rows gain the tag); an acked action keeps its ✓ and the same neutral dashed rule. Neutral, not amber: amber means "cannot confirm" and a shadow is not that (the CIRCE `shadow-banner` tier keeps amber for its warning). The original finding: **The River `.shadow` tier dims text below AA.** `.rv-item.shadow { opacity: 0.5 }` (`v2.css`) puts `--text` at **4.49 : 1** and `--text-2` at **2.48 : 1** on `--panel`, and it applies to every TRITON / HERA shadow row today. The dimming is how "shadow" is shown, so changing it is a HELIOS call. CIRCE'S STEW avoids it with the `shadow-banner` tier (§7.8), and the same treatment (a dashed rule instead of opacity) would fix the existing shadow rows.
